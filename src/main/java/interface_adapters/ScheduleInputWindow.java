...
public abstract class ScheduleInputWindow extends Window{
    ...
    protected String[] getTimes(Scanner scanner) {
        ...
        for (int i = 0; i < howManyTimesInt; i++){
            while(!super.checker.isValidTime(time)){
    ...
    public String selectDay(Scanner scanner){
        String day = "";
        while (!super.checker.isNumeric(day) || 0 > Integer.parseInt(day) || Integer.parseInt(day)  > 32){
    ...
    public String selectMonth (Scanner scanner) {
        String month = "  ";
        while (!super.checker.isNumeric(month) || 0 > Integer.parseInt(month) || Integer.parseInt(month)  > 12 ||
                month.length() != 2){
    ...
    public String selectHowManyTimes(Scanner scanner) {
        ...
        while (true) {
            ...
            String input = scanner.nextLine();
            if (super.checker.isNumeric(input)) {
                return input;
            }
        }
    }
